...
const ServicesPage = () => {
  ...
  useEffect(() => {
    const buildServicesData = () => {
      // Define main categories with their slugs and which services belong to them
      const categoryDefinitions = [
        {
          ...
          featured: true,
          // Services that belong under this category (subcategory slugs)
      ...
      // Build a set of all known subcategory slugs from category definitions
      const knownSubcategorySlugs = new Set();
      categoryDefinitions.forEach(cat => {
        cat.categorySlugs.forEach(slug => knownSubcategorySlugs.add(slug));
      });
      ...
      // Array for services that don't belong to any main category
      const otherServices = [];

      // Process all auto-generated services
      autoGeneratedServices.forEach(slug => {
        // Skip main category pages themselves
        const isMainCategory = categoryDefinitions.some(cat => cat.slug === slug);
        if (isMainCategory) return;

        // Check if this service belongs to any category
        let belongsToCategory = false;
        
        // First, check if it's explicitly listed in categorySlugs
        ...
        // If not explicitly categorized, try to infer from slug patterns
        if (!belongsToCategory) {
          ...
          if (slug.includes('skin') || slug.includes('laser') || slug.includes('botox') || 
              ...
              slug.includes('mesotherapy') || slug.includes('microdermabrasion') ||
              slug.includes('pigmentation') || slug.includes('dark-circle') ||
              slug.includes('carboxy') || slug.includes('electrolysis') ||
              slug.includes('derma') || slug.includes('carbon') || slug.includes('fractional') ||
              slug.includes('birthmark') || slug.includes('hair-removal') || slug.includes('bleaching') ||
              slug.includes('mole') || slug.includes('nail-fungus') || slug.includes('scar') ||
              slug.includes('spider-veins') || slug.includes('tattoo') || slug.includes('hair-growth') ||
              slug.includes('hair-loss') || slug.includes('hair-prp') || slug.includes('eximia') ||
              slug.includes('iv-drip') || slug.includes('body-contouring') || slug.includes('cellulite') ||
              slug.includes('belly-fat') || slug.includes('female-genital')) {
            servicesByCategory['aesthetic-dermatology-dubai'].push({
              ...
              description: `Professional ${formatServiceName(slug).toLowerCase()} treatment`
                     ...
                     slug.includes('diabetes') || slug.includes('thyroid') || slug.includes('gut-health') ||
                     slug.includes('panchakarma') || slug.includes('gastrointestinal') ||
                     slug.includes('shirodhara') || slug.includes('abhyanga') || slug.includes('basti') ||
                     slug.includes('nasya')) {
          ...
          } else if (slug.includes('ultrasound') || slug.includes('electrotherapy') ||
                     slug.includes('pelvic') || slug.includes('scoliosis') || slug.includes('functional') ||
                     slug.includes('dry-needling') || slug.includes('back-pain') || slug.includes('knee-pain') ||
                     slug.includes('migraine') || slug.includes('office-neck') || slug.includes('post-surgery') ||
          ...
          } else if (slug.includes('teeth') || slug.includes('dental') || slug.includes('veneers') ||
                     slug.includes('root-canal') || slug.includes('snap-on') || slug.includes('bleaching') ||
                     slug.includes('dentures') || slug.includes('composite') || slug.includes('braces') ||
                     slug.includes('crown') || slug.includes('scaling') || slug.includes('gum')) {
            servicesByCategory['dental-dubai'].push({
              name: formatServiceName(slug),
              slug: slug,
              description: `Professional ${formatServiceName(slug).toLowerCase()} treatment`
            });
          } else if (slug.includes('gastrointestinal') || slug.includes('allergy') ||
                     slug.includes('wound') || slug.includes('injury') || slug.includes('infection') ||
                     slug.includes('dyslipidemia') || slug.includes('slimming') || slug.includes('thyroid') ||
                     slug.includes('asthma') || slug.includes('copd') || slug.includes('diabetes') ||
                     slug.includes('hypertension') || slug.includes('check-up') || slug.includes('examination')) {
            servicesByCategory['general-physician-dubai'].push({
              name: formatServiceName(slug),
              slug: slug,
              description: `Professional ${formatServiceName(slug).toLowerCase()} treatment`
            });
          } else if (slug.includes('hydra') || slug.includes('prp') || slug.includes('golden') ||
                     slug.includes('exosomes') || slug.includes('oxygeneo')) {
            servicesByCategory['facial-dubai'].push({
              name: formatServiceName(slug),
              slug: slug,
              description: `Professional ${formatServiceName(slug).toLowerCase()} treatment`
            });
          } else {
            // Add to other services if no category matches
            otherServices.push({
              name: formatServiceName(slug),
              slug: slug,
              description: `Professional ${formatServiceName(slug).toLowerCase()} treatment`
      ...
      // If there are uncategorized services, add them as "Other Services"
      ...
      // Build final services data structure
      const finalServicesData = categoryDefinitions.map(cat => ({
        id: cat.id,
        name: cat.name,
        slug: cat.slug,
        icon: cat.icon,
      ...
      setServicesData(finalServicesData);
    };
